import cv2
import mediapipe as mp
import time


class handDetector():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackCon=0.5):
        self.mode = mode
        self.maxHands = maxHands
        self.detectionCon = detectionCon
        self.trackCon = trackCon

        self.mpHands = mp.solutions.hands
        self.hands = self.mpHands.Hands(self.mode, self.maxHands,
                                        self.detectionCon, self.trackCon)
        self.mpDraw = mp.solutions.drawing_utils

    def findHands(self, img, draw=True):
        imgRGB = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        #imgRGB=cv2.flip(imgRGB,1)
        self.results = self.hands.process(imgRGB)
        # print(results.multi_hand_landmarks)

        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img, handLms,
                                               self.mpHands.HAND_CONNECTIONS,
                                               self.mpDraw.DrawingSpec(color=(121, 22, 76), thickness=2, circle_radius=4),
                                        self.mpDraw.DrawingSpec(color=(250,222, 250), thickness=2, circle_radius=4))
        if cv2.waitKey(10) & 0xFF == ord('q'):
            cv2.release()

        return img

    def findPosition(self, img, handNo=0, draw=True):

        lmList1 = []
        lmList1_x=[]
        lmList1_y=[]
        left=0
        right=0
        #lmList2 = []
        if self.results.multi_hand_landmarks:
            myHand1 = self.results.multi_hand_landmarks[handNo]
            start=0

            for id, lm in enumerate(myHand1.landmark):
                # print(id, lm)
                h, w, c = img.shape
                if start==0:
                    x_ref=lm.x
                    y_ref=lm.y
                    start=start+1
                if start==1:
                    val=(lm.x)-x_ref
                    if(val<0):
                        right,left=1,0
                    else:
                        left,right=1,0
                cx, cy = int((lm.x-x_ref) * w), int((lm.y-y_ref) * h)
                # print(id, cx, cy)
                lmList1.append([id, cx, cy])
                lmList1_x.append(cx)
                lmList1_y.append(cy)

                if draw:
                    cv2.circle(img, (cx, cy), 15, (255, 0, 255), cv2.FILLED)
            #myHand2 = self.results.multi_hand_landmarks[1]
            #print(myHand1)
            #print('+\n')
            #print(myHand2)
            #print('  \n')
            '''for id, lm in enumerate(myHand2.landmark):
                # print(id, lm)
                h, w, c = img.shape
                cx, cy = int(lm.x * w), int(lm.y * h)
                # print(id, cx, cy)
                lmList2.append([id, cx, cy])
                if draw:
                    cv2.circle(img, (cx, cy), 15, (255, 0, 255), cv2.FILLED)

        lmList=[lmList1,lmList2]'''
                
        return lmList1, lmList1_x, lmList1_y, right,left


def main():
    pTime = 0
    cTime = 0
    cap = cv2.VideoCapture(0)
    detector = handDetector()
    while True:
        success, img = cap.read()
        img = detector.findHands(img)
        lmList = detector.findPosition(img)
        if len(lmList) != 0:
            print(lmList[4])

        cTime = time.time()
        fps = 1 / (cTime - pTime)
        pTime = cTime

       

        cv2.imshow("Image", img)
        cv2.waitKey(1)
        


if __name__ == "__main__":
    main()